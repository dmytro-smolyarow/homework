...
import { favoriteIdsQueryOptions, useToggleFavoriteMutation } from '@/app/entities/api/favorites'
...
const FavoriteButton: FC<Readonly<IProps>> = (props) => {
  ...
  if (!session) {
    return (
      <button className='btn fav' disabled={isPending} onClick={() => router.push('/login?redirect=/favorites')}>
        ☆ Log in to favorite
      </button>
    )
  }
